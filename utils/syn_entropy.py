...
def run_tshark(pcap_path, syn_only=False):
    """
    Extract:
...
def compute_entropy_series(rows, interval, window_bins, dst_prefix=None):
    if not rows:
        return [], [], []
...
def main():
    parser = argparse.ArgumentParser(description="Compute entropy graphs from ns-3 SYN flood PCAP")
    parser.add_argument("--pcap", required=True,
                        help="PCAP path or glob, e.g. results/synflood/pcap/synflood_core_r0-*.pcap")
    ...
    parser.add_argument("--dst-prefix", default="10.2.",
    ...
    rows = run_tshark(pcap, syn_only=args.syn_only)
